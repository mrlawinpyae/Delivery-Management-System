...
const tokens = {
  dark: {
    ...
    // Toggle button
    toggleBtn:
      "bg-slate-800/70 text-slate-400 hover:bg-slate-700/80 hover:text-slate-200 border-slate-700/50",
  ...
  return (
    ...
    >
      <div className={`relative ${sm ? "h-1.5 w-1.5" : "h-2 w-2"}`}>
        ...
        <span
          className={`relative block ${sm ? "h-1.5 w-1.5" : "h-2 w-2"} rounded-full bg-emerald-400 shadow-sm shadow-emerald-400/50`}
        />
      </div>
      <span
  ...
  return (
    ...
    >
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`relative flex h-12 w-12 items-center justify-center rounded-2xl transition-all duration-200 ${isActive ? tk.iconActive : tk.iconInactive}`}
                >
                ...
                {/* Hover tooltip */}
                <div
                  className={`pointer-events-none absolute top-1/2 left-14 -translate-y-1/2 rounded-lg border px-2.5 py-1 text-[11px] font-medium whitespace-nowrap opacity-0 shadow-xl backdrop-blur-sm transition-opacity duration-150 group-hover:opacity-100 ${tk.tooltip}`}
                >
                  {item.label}
                </div>
              </Link>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-sm font-semibold tracking-wide ${tk.pageLabel}`}
            >
              {activeLabel}
            </span>
            <span
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 md:hidden">
        ...
        <div
          className={`h-px bg-gradient-to-r from-transparent ${tk.navEdgeLine} to-transparent`}
        />

        <nav
          className="flex items-center justify-around px-4 py-3"
          style={{
            background: tk.mobileNav,
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                />
                <span
                  className={`relative z-10 text-[10px] font-semibold tracking-wide transition-colors duration-200 ${isActive ? tk.mobileLabelActive : tk.mobileLabelInactive}`}
                >
                  {item.label}
                </span>
              </Link>
