  ...
  return (
    <div className="space-y-10">
      ...
      <section className="space-y-4">
        {/* မင်းရဲ့ ရှိပြီးသား Hero Welcome Section ကို ဒီနေရာမှာ ပြန်ထည့်ပါ */}
      </section>

      {/* ─── RESTAURANT GRID SECTION ─── */}
      <section className="space-y-6">
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {restaurantList.map((shop: any, index: number) => (
            <motion.div
              key={shop._id}
                ...
                <Card className="group relative h-80 overflow-hidden rounded-3xl border-0 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.15)]">
                  ...
                  <CardContent className="absolute right-0 bottom-0 left-0 z-10 space-y-4 p-5 text-white">
                    <div className="space-y-1">
                      <h3 className="font-serif text-2xl font-bold tracking-tight text-white drop-shadow-md transition-colors group-hover:text-amber-400">
                        {shop.name}
                      </h3>
                    ...
                    {/* Footer Metadata - Only "Open Now" */}
                    <div className="flex items-center text-xs">
                      <span className="flex items-center gap-1.5 font-medium text-emerald-400 drop-shadow-sm">
